...
const ld=async(k)=>{try{const r=await window.storage.get(k);return r?JSON.parse(r.value):null}catch(e){return null}};
const sv=async(k,d)=>{try{await window.storage.set(k,JSON.stringify(d))}catch(e){console.error(e)}};
...
export default function App(){
  ...
  useEffect(()=>{(async()=>{const s=await ld("griht-w");const a=await ld("griht-a");if(s)setWks(s);if(a){setAw(a);setView("active")}setLoad(false)})()},[]);
  ...
  const pa=useCallback(async(w)=>{setAw(w);await sv("griht-a",w)},[]);
  const startW=async(nm)=>{const w={id:uid(),nm,st:new Date().toISOString(),exs:[]};await pa(w);setView("active")};
  const finishW=async()=>{if(!aw)return;const x={...aw,et:new Date().toISOString()};const u=[x,...wks];setWks(u);await sv("griht-w",u);await sv("griht-a",null);setAw(null);setEl(0);setView("home")};
  const discardW=async()=>{await sv("griht-a",null);setAw(null);setEl(0);setView("home")};
  const deleteW=async(id)=>{const u=wks.filter((w)=>w.id!==id);setWks(u);await sv("griht-w",u);setView("history")};
  if(load)return (<div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",background:P.bg,fontFamily:P.ft}}><HexMark size={56}/><div style={{fontSize:32,fontWeight:900,color:P.tx,letterSpacing:6,marginTop:16}}>GRI<span style={{color:P.acc2}}>HT</span></div><div style={{fontSize:9,color:P.tx3,letterSpacing:4,fontWeight:700,marginTop:8}}>PUT IN WORK</div></div>);
  if(view==="landing")return <Landing onLaunch={()=>setView("home")}/>;
  return (
    <div style={{fontFamily:P.ft,background:P.bg,color:P.tx,minHeight:"100vh",maxWidth:480,margin:"0 auto"}}>
      {view==="home"&&<Home wks={wks} go={setView} hasAw={!!aw}/>}
...
function Landing({onLaunch}){
  ...
  return (
    <div style={{fontFamily:P.ft,background:P.tx,color:P.bg,minHeight:"100vh",display:"flex",flexDirection:"column"}}>
      ...
      {/* CTA */}
      <div style={{padding:"0 24px 60px",maxWidth:400,margin:"0 auto",width:"100%",opacity:vis?1:0,transform:vis?"translateY(0)":"translateY(30px)",transition:"all 0.8s 0.4s cubic-bezier(0.16,1,0.3,1)"}}>
        <button onClick={onLaunch} style={{
          width:"100%",padding:18,borderRadius:12,border:"none",cursor:"pointer",
          background:P.bg,color:P.tx,fontSize:14,fontWeight:900,fontFamily:P.ft,
          letterSpacing:4,transition:"transform 0.15s",
        }} onMouseDown={(e)=>e.currentTarget.style.transform="scale(0.97)"} onMouseUp={(e)=>e.currentTarget.style.transform="scale(1)"}>
          LAUNCH APP
        </button>
        <div style={{textAlign:"center",marginTop:16}}>
          <span style={{fontSize:10,color:"rgba(240,237,232,0.2)",letterSpacing:2,fontWeight:600}}>FREE · NO ACCOUNT · WORKS OFFLINE</span>
        </div>
      </div>
